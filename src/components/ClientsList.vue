<template>
    <div class="filters">
        ...
        <el-input style="width: 150px;" type="text" v-model="filter.fullname" placeholder="Иван Иванов" @keydown.enter.prevent="sync(true)"/>
        <span>Номер телефона: </span>
        <el-input style="width: 150px;" type="text" v-maska="'+7 (###) ### ##-##'" v-model="filter.phone" placeholder="Номер телефона" @keydown.enter.prevent="sync(true)"/>
    </div>
    ...
    <div class="convs-list" v-if="ready" @scroll="listScroll" ref="list">
        ...
        >
            ...
            <div class="item-data" v-if="activeMember==ind">
                ...
                <div class="item-row" v-if="i.fullname.length > 0">
            ...
            rows: [
                {
                    width: '175px',
                    name: 'ID клиента',
                    return: (i) => `Клиент <strong>#${i.id}</strong>`
                },
                {
                    width: '200px',
                    name: 'Имя',
                    return: (i) => i.fullname ? i.fullname : mask(i.phone, '+7 (7##) ### ##-##')
                },
            ],
            moment
        }
        ...
        async sync(reset) {
            return new Promise((resolve) => {
                (async () => {
                    ...
                    this.loading = true;
                    let response = await this.serverQuery('admin', 'getAllClients', {offset: this.offset, fullname: this.filter.fullname, phone: this.filter.phone});
                    this.loading = false;
